...
Create Kafka Topic
	kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 1 --topic pyspkaf_test
...
pyKafStream = KafkaUtils.createStream(ssc = KStreamContext,zkQuorum = 'localhost:2181',groupId = 'pyspkaf_testGroup',topics = {'pyspkaf_test':1})

sumedData = pyKafStream.map(lambda data : stringToNumberSum(data[1]))
